...
def renderizaMandelbrot(x1, y1, x2, y2, ancho, iteraciones, nombreFicheroPNG):
	...
	# image size
	imgx = int(ancho)
	imgy = int(abs (float(y2) - float(y1)) * float(ancho) / abs(float(x2) - float(x1)))
...
def getColorPaleta(paleta, nColoresPaleta, color):
		nuevoColor = int(color) % (int(nColoresPaleta))
		...
		for y in range(imgy):
			...
			for x in range(imgx):
				...
				if (i == maxIt - 1):
					...
					pal=getColorPaleta(paleta, nColoresPaleta, i)
					image.putpixel((x, y), (pal[0], pal[1], pal[2]))
